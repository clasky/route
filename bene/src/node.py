...
class Node(object):
    ...
    def receive_packet(self,packet):
        ...
        if packet.destination_address == 0:
            Sim.broadcastTrace("%s received packet" % (self.hostname))
        ...
        if packet.ttl <= 0:
            Sim.broadcastTrace("%s dropping packet due to TTL expired" % (self.hostname))
            return
    ...
    def forward_broadcast_packet(self,packet):
        for link in self.links:
            Sim.broadcastTrace("%s forwarding broadcast packet to %s" % (self.hostname,link.endpoint.hostname))
